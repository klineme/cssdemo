...
window.onload = function(){
    (function(MenuMod, CartMod){
        //Get Menu, then display it
        constructMenu(MenuMod.getMenu());
        constructCart(CartMod.getWebCart());
        updateTotal(CartMod.calcTotal());
        var j = JSON.stringify(MenuMod);
        c.log(MenuMod);
    ...
    })(window.Menu || (window.Menu = {}), (window.ShoppingCart || (window.ShoppingCart = {})));//IIFE
}
...
function cleanCartNodes(CartArray){ //Specifically for when I add or delete nodes
    for(i = 0; i < CartArray.length; i++){
        var doesExist = document.getElementById("cartNode"+i.toString());
        if(doesExist){ //Helps prevent duplicates and ghosting
